...
nutrients <- read.csv("nutrients_in_oils.csv")
nutrients$Oil.type <- c("Palm Oil\n(refined)", "Palm Oil\n(hydrogenated)", "Rapeseed\nOil", "Olive\nOil", "Sunflower\nOil", "Coconut\nOil")
...
ggplot(nutrients_with_na, aes(x=as.factor(id), y=grams, fill = Fatty.acids)) +
  ...
  geom_text(aes(x=id, y=grams+12, label=grams), 
            color="white", fontface="bold",alpha=0.6, size= 4, 
            inherit.aes = FALSE) +
  geom_segment(data=base_data, aes(x = start, y = -5, xend = end, yend = -5), colour = "white", alpha=0.8, size=0.8 , inherit.aes = FALSE )  +
  geom_text(data=base_data, aes(x = title, y = -18, label=Oil.type), hjust = c(0.2,0.4,0.92,0.8,0.85,0.15), colour = "white", alpha= 1.2, size=4.5, fontface = "bold", inherit.aes = FALSE) +
