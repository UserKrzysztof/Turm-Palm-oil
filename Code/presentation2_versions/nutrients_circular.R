...
ggplot(nutrients_with_na, aes(x=as.factor(id), y=grams, fill = Fatty.acids)) +
  ...
  geom_segment(data=grid_data, aes(x = end, y = 80, xend = start, yend = 80), linetype = "dashed", colour = "darkgrey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_data, aes(x = end, y = 60, xend = start, yend = 60), linetype = "dashed", colour = "darkgrey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_data, aes(x = end, y = 40, xend = start, yend = 40), linetype = "dashed", colour = "darkgrey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  geom_segment(data=grid_data, aes(x = end, y = 20, xend = start, yend = 20), linetype = "dashed", colour = "darkgrey", alpha=1, size=0.3 , inherit.aes = FALSE ) +
  annotate("text", x = rep(max(nutrients_with_na$id),4), y = c(20, 40, 60, 80), label = c("20", "40", "60", "80") , color="grey", size=3.5 , angle=0, fontface="bold", hjust=0.3) +
  ...
  theme(
    ...
    legend.title = element_text(size = 15, family = "bold")
  ) +
  coord_polar() +
  ...
  geom_text(data=base_data, aes(x = title, y = -18, label=Oil.type), hjust = c(0.15,0.4,0.9,0.68,0.7,0.17), vjust = c(0.7,0.3,0.5,0.9,0.25,0.5), colour = "white", alpha= 1.2, size=4.5, fontface = "bold", inherit.aes = FALSE) +
  scale_fill_manual(values = c("#FF8700", "#FFFC30", "#EC1C1C")) +         # coco (-p)(-g), olive, hydro, ref, rape, sun
  labs(title = "The content of fatty acids by oil type",
       fill = "Fatty acids:\n  (g/100g)") +
