...
nutrients <- read.csv("H:\\Documents\\Projekt TWD\\nutrients_in_oils2.csv")
...
# Poprawna kolejność: "Coconut oil", "Sunflower oil", "Olive oil", "Rapeseed oil", "Palm oil (hydrogenated)", "Palm oil (refined)"

fatty_acids$Oil.type <- as.factor(fatty_acids$Oil.type)
levels(fatty_acids$Oil.type)
fatty_acids$Oil.type <- factor(fatty_acids$Oil.type, levels = levels(fatty_acids$Oil.type)[c(1, 2, 3, 4, 6, 5)])

omega_3$Oil.type <- as.factor(omega_3$Oil.type)
levels(omega_3$Oil.type)
omega_3$Oil.type <- factor(omega_3$Oil.type, levels = levels(omega_3$Oil.type)[c(1, 2, 3, 4, 6, 5)])

vitamin_e$Oil.type <- as.factor(vitamin_e$Oil.type)
levels(vitamin_e$Oil.type)
vitamin_e$Oil.type <- factor(vitamin_e$Oil.type, levels = levels(vitamin_e$Oil.type)[c(1, 2, 3, 4, 6, 5)])
...
w1 <- ggplot(fatty_acids, aes(x = Nutrients, y = Oil.type, fill=grams)) + 
  ...
  theme(axis.title = element_blank(),
        ...
        legend.title = element_text(color = "white", size = 14, vjust = 0.9),
...
# Połączenie trzech wykresów w jeden - wersja 1
final_plot <- (w1 + w2 + w3) + patchwork::plot_layout(widths = c(0.7, 0.22, 0.22)) +
  labs(title = "Nutrients content by oil type") +
  theme(plot.title = element_text(hjust = 8.4, vjust = 6, color = "white", size = 17))
final_plot

# Połączenie trzech wykresów w jeden - wersja 2
final_plot <- (w1 + w2 + w3) + patchwork::plot_layout(widths = c(0.7, 0.22, 0.22)) +
  labs(title = "Nutrients content by oil type") +
  theme(plot.title = element_text(hjust = 9, vjust = 6, color = "white", size = 17))
final_plot
